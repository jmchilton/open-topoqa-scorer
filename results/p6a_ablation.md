...
rescuing top-1 ranking loss. There is **no uniform "topology adds signal"** — the honest statement is
"topology adds *generalization* to interface rank-ordering, and conventional features are a strong
baseline." Together with P6b (a free pLDDT baseline ties top-1), the two halves converge: the
topological approach's genuine value is generalizable *ranking quality*, not the top-1 numbers the
literature leads with.
